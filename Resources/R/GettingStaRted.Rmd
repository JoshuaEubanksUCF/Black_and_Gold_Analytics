...
You can also select more than one column/row by creating a vector of values:

```{r}
mtcars[1,c(1,2,4,5)]

mtcars[1,1:5] # This grabs the first 5 columns
```


## Creating New Column

You will often times need to transform one column into another. Suppose we want the logarithm of horsepower
...
If you need to change locations, you can easily do that as well using `setwd()`.
...
Depending upon the filetype, you may need special packages in order to import the data. For this example, since it is an excel file, we will need to import a new package `install.packages("readxl")` .

You will need to change the path to the file, but here is my path as an example:

```{r}
library(readxl)

Wealth1Percent <- read_excel("C:/Users/jo585802/OneDrive - University of Central Florida/Courses/ECP3203/CourseNotes/Coding Example/Introduction_to_R/Wealth1percent.xlsx")
```

# Base Plots

We can easily plot this information in the base R package. There are many other ways to do this, but for now, we can use the default.
...
plot(Wealth1Percent$quarter,Wealth1Percent$Share,
     ...
     xlab = "",
     ylab = "Share [%]",
     main = "Share of Wealth Held by the 1%")
```
